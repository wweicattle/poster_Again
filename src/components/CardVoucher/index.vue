<template>
  <div id="card-vouchar-contain">
    <div class="birth-back">
      <div class="member-back">
        <span class="member-birth-title">卡券到期提醒</span
        ><span class="member-birth-content"
          >提醒会员卡券即将到期，提高消费</span
        >
...
export default {
  ...
  methods: {
    ...
    slectIndexBtn(index) {
      ...
      // 切换type进行重新请求
      this.getCardOvertimelist({ cid: this.cid, type: this.selectindex });
    },
    getTalkSkill(obj) {
  ...
  mounted() {
    ...
    eventBus.$on("addRecord", (obj) => {
      if (this.$route.path == "/cardvouchar") {
